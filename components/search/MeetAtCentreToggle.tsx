...
export default function MeetAtCentreToggle({ value, onChange }: MeetAtCentreToggleProps) {
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-gradient-to-r from-gray-50 to-emerald-50 rounded-xl shadow-sm">
      <div className="md:max-w-[70%]">
        <div className="flex items-center">
          {value ? (
            <MapPin className="h-6 w-6 text-emerald-500 mr-2" />
          ) : (
            <Navigation className="h-6 w-6 text-emerald-500 mr-2" />
          )}
        ...
        {!value && (
          <div className="mt-2 text-sm text-emerald-700 flex items-center">
      ...
      <div className="flex flex-col items-end mt-4 md:mt-0">
        ...
        <div className="mt-2 text-center text-xs font-medium">
          <span className={!value ? 'text-emerald-600' : 'text-gray-500'}>Pickup service</span>
